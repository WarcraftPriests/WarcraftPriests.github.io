...
var WCP_Chart = function WCP_Chart(id, options) {
  ...
  this.chartOptions = {
    ...
    title: {
      ...
      text: "Ancient Maddness - Auspicious Spirits - Trinkets - Composite"
    },
    plotOptions: {
      series: {
        stacking: 'normal',
...
WCP_Chart.prototype.init = function() {
  ...
  this.updateStackedBarChart("trinkets", "Composite", "am-as"); // Setup the initial chart
};
...
WCP_Chart.prototype.updateStackedBarChart = function(simsBtn, fightStyle, talentChoice, covenantType) {
  ...
  jQuery.getJSON(baseurl + "/" + simsBtn + "/results/Results_" + fightStyle +  "_" + talentChoice + covenantPath +  ".json", function(data) {
    ...
    for(currStep of steps) {
      let maxStep = data["simulated_steps"][data["simulated_steps"].length-1];
      let stepDpsValues = [];
      for(sortedData of dpsSortedData) {
        ...
        if(dps >= 0) {
          if(currStep == minStep) {
            ...
            stepDpsValues.push(dps-data["data"][sortedData][currStep - stepDifference]);
          }
        } else {
    ...
    document.getElementById(this.chartId).style.height = 200 + dpsSortedData.length * 30 + "px"; // Size the chart by our data.
...
WCP_Chart.prototype.updateSingleBarChart = function(simsBtn, fightStyle, talentChoice, covenantType) {
  ...
  jQuery.getJSON(baseurl + "/" + simsBtn + "/results/Results_" + fightStyle +  "_" + talentChoice + covenantPath +  ".json", function(data) {
    ...
    document.getElementById(this.chartId).style.height = 200 + dpsValues.length * 30 + "px";
    this.chart.setSize(document.getElementById(this.chartId).style.width, document.getElementById(this.chartId).style.height);
...
function itemClick(clicked) {
  if (clicked == 'soulbind-Kyrian' || clicked == 'soulbind-Necrolord' || clicked == 'soulbind-Night_Fae' || clicked == 'soulbind-Venthyr') {
    itemBtn = 'soulbinds'
    covenantBtn = clicked.split("-")[1].toLowerCase();
    wcp_charts.updateSingleBarChart("soulbinds", fightBtn, talentsBtn, covenantBtn);
  } else if (clicked == 'conduit-Kyrian' || clicked == 'conduit-Necrolord' || clicked == 'conduit-Night_Fae' || clicked == 'conduit-Venthyr') {
    itemBtn = 'conduits';
    covenantBtn = clicked.split("-")[1].toLowerCase();
    wcp_charts.updateStackedBarChart("conduits", fightBtn, talentsBtn, covenantBtn);
  } else if (clicked == 'legendaries-Kyrian' || clicked == 'legendaries-Necrolord' || clicked == 'legendaries-Night_Fae' || clicked == 'legendaries-Venthyr') {
    itemBtn = 'legendaries';
    covenantBtn = clicked.split("-")[1].toLowerCase();
    wcp_charts.updateSingleBarChart("legendaries", fightBtn, talentsBtn, covenantBtn);
...
function read_config(cfgfile){
  jQuery.get({url: cfgfile, dataType: "text"})
    .done(function (data) {
      ...
      for(i in builds)
      {
        ...
        let secondTalent = GetTalentName(talent[1]);
        talentBtns.push(firstTalent + " - " + secondTalent);
        
        for(f in builds[i])
        {
          let fightstyle = f.charAt(0).toUpperCase() + f.slice(1);
          titles.push(firstTalent + " - " + secondTalent + " - " + fightstyle);
...
function createTalentButtons(buttonArray)
{
  ...
  for(b in buttonArray)
  {
    ...
    let secondTalent = GetTalentShortName(talentSplit[1].trim());
    let shorthandTalent = firstTalent+"-"+secondTalent;
...
function createSimsButtons(simTypeArray)
{
  ...
  for(s in simTypeArray)
  {
    ...
    let simBtnTextCapitalized = simTypeArray[s].charAt(0).toUpperCase() + simTypeArray[s].slice(1);
